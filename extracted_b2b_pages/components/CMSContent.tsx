...
import PrintMediaContent from './PrintMediaContent';
...
export default function CMSContent() {
  const [activeTab, setActiveTab] = useState<'website' | 'print'>('website');

  return (
    <div className="w-full">
        <div className="flex space-x-8 mb-12 border-b border-white/10 pb-4">
            <button 
                onClick={() => setActiveTab('website')}
                className={`text-lg font-light transition-colors ${activeTab === 'website' ? 'text-white border-b border-white' : 'text-neutral-500 hover:text-white'}`}
            >
                Website Hosting & CMS
            </button>
            <button 
                onClick={() => setActiveTab('print')}
                className={`text-lg font-light transition-colors ${activeTab === 'print' ? 'text-white border-b border-white' : 'text-neutral-500 hover:text-white'}`}
            >
                Print Media
            </button>
        </div>
        ...
        {activeTab === 'print' && <PrintMediaContent />}
    </div>
  );
}
